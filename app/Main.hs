...
fileExecution path = do { file <- readFileEither path; case file of {
  ...
  Right content -> case runParser parseCodeToGomExpr content of {
    ...
    Right (out, _) -> case convertListToAST internalEnv out of {
      EvalResult (Left (EvalError msg _)) -> putStrLn msg >> exitWith
        (ExitFailure 84);
      EvalResult (Right (newEnv, asts)) -> print newEnv >> print asts; } } } }
